...
static const char* SETTING_ITEMS[] = {"Status Page", "Invert Encoder", "Beeper Test", "About", "Back"};
...
void DuenderUI::drawStatus() {
  ...
  drawFooter(WiFi.localIP().toString().c_str());
  u->sendBuffer();
}
...
void DuenderUI::drawSystem() {
  ...
  u->drawStr(0, 47, (String("IP ") + WiFi.localIP().toString()).c_str());
